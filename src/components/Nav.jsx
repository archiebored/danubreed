  ...
  return (
    <>
      <nav className="fixed top-3.5 left-1/2 -translate-x-1/2 z-50 w-[calc(100%-28px)] max-w-[400px] sm:max-w-[640px] lg:max-w-[820px] flex items-center justify-between gap-2 rounded-full border border-black/10 dark:border-white/10 bg-base-light/85 dark:bg-[#0e0e0e]/85 backdrop-blur-xl px-4 sm:px-5 py-2 shadow-lg shadow-black/10 dark:shadow-black/30">
        <span className="font-display text-lg tracking-wide text-accent flex-shrink-0">Da Nu Breed</span>
        ...
        <div className="flex items-center gap-1.5 flex-shrink-0">
          <button
            ...
            className="w-8 h-8 rounded-full border border-black/10 dark:border-white/10 flex items-center justify-center text-muted-light dark:text-muted-dark"
          >
      ...
      {open && (
        <div className="sm:hidden fixed inset-0 z-50 bg-base-light dark:bg-base-dark flex flex-col gap-2 px-6 py-[90px]">
          <button
            ...
            className="absolute top-4 right-4 w-9 h-9 rounded-full border border-black/10 dark:border-white/10 flex items-center justify-center"
          >
            <X size={18} />
          </button>
          {links.map((l) => (
            <NavLink
              ...
              onClick={() => setOpen(false)}
              className={({ isActive }) =>
                `px-5 py-4 rounded-2xl text-base font-semibold border ${
                  isActive
                    ? 'text-accent border-accent/30 bg-accent/10'
                    : 'text-muted-light dark:text-muted-dark border-black/10 dark:border-white/10 bg-surface-light dark:bg-surface-dark'
                }`
              }
            >
              {l.label}
