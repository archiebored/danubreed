...
export default function StaffServiceTimes() {
  ...
  const [form, setForm] = useState({ label: '', time_text: '', sort_order: 0 });
  ...
  function startEdit(item) {
    ...
    setForm({ label: item.label, time_text: item.time_text, sort_order: item.sort_order });
  }

  function resetForm() {
    setEditingId(null);
    setForm({ label: '', time_text: '', sort_order: items.length });
  }
  ...
  async function handleSubmit(e) {
    ...
    if (editingId) {
      ...
      await supabase.from('service_times').insert(form);
    }
    setSaving(false);
    resetForm();
    load();
  ...
  return (
    <div>
      ...
      <p className="font-display text-4xl tracking-wide mb-5">
        {editingId ? 'Edit service time' : 'Add service time'}
      </p>
      <form
      ...
      >
        <input
          type="text"
          required
          placeholder="Label, e.g. Bible study"
          value={form.label}
        ...
        {items.map((item) => (
          ...
          >
            ...
            <div className="flex gap-3 text-xs font-medium">
              <button onClick={() => startEdit(item)} className="text-accent">
                Edit
              </button>
              <button onClick={() => remove(item.id)} className="text-red-400">
                Delete
              </button>
            </div>
          </div>
