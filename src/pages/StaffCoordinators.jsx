...
export default function StaffCoordinators() {
  ...
  function resetForm() {
    setEditingId(null);
    setForm({ full_name: '', role_label: '', phone: '', whatsapp_link: '', sort_order: items.length });
  }
  ...
  async function handleSubmit(e) {
    ...
    setSaving(true);
    if (editingId) {
      await supabase.from('coordinators').update(form).eq('id', editingId);
    } else {
      await supabase.from('coordinators').insert(form);
    }
    setSaving(false);
    resetForm();
    load();
  ...
  async function remove(id) {
    if (!confirm('Remove this coordinator from the public directory?')) return;
