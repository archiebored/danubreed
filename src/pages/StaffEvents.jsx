...
import { Calendar, MapPin, ChevronDown, ChevronUp, Search, Users } from 'lucide-react';
...
export default function StaffEvents() {
  ...
  async function loadAll() {
    ...
    const { data: regs } = await supabase
      ...
      .select('event_id, full_name, phone_number');
    const grouped = {};
    (regs || []).forEach((r) => {
  ...
  return (
    <div>
      <p className="text-[11px] font-bold uppercase tracking-[2px] text-accent mb-1.5">
      ...
      >
        ...
        <div className="flex gap-2">
          <input
            ...
            className="flex-1 rounded-md bg-base-dark border border-white/10 px-3 py-2 text-sm outline-none"
          />
          <input
            type="number"
            min="0"
            ...
            className="w-28 rounded-md bg-base-dark border border-white/10 px-3 py-2 text-sm outline-none"
          />
        </div>
        <input
          type="text"
        ...
        <div className="flex gap-2 mt-1">
          <button
            type="submit"
            disabled={saving}
            className="bg-accent text-[#1a0a00] rounded-md py-2 px-4 text-sm font-bold disabled:opacity-60"
          >
            {saving ? 'Saving...' : editingId ? 'Save changes' : 'Create event'}
          </button>
          {editingId && (
            <button type="button" onClick={resetForm} className="text-sm text-muted-dark">
              Cancel
            </button>
          )}
        </div>
      ...
      <div className="flex items-center justify-between mb-4 flex-wrap gap-3">
        ...
        <div className="relative">
          ...
          <input
            ...
            className="rounded-full bg-surface-dark border border-white/10 pl-9 pr-4 py-2 text-sm outline-none w-52"
          />
        </div>
      </div>
      ...
      <div className="flex flex-col gap-2.5 max-w-2xl">
        {filtered.map((ev) => {
          ...
          return (
            <div key={ev.id} className="rounded-xl border border-white/10 bg-surface-dark p-4">
              <div className="flex justify-between items-start gap-3">
                ...
                <div className="flex gap-3 text-xs font-medium flex-shrink-0">
                  <button onClick={() => startEdit(ev)} className="text-accent">
                    Edit
                  </button>
                  <button onClick={() => removeEvent(ev.id)} className="text-red-400">
                    Delete
                  </button>
                </div>
              </div>
              ...
              {ev.capacity ? (
                <div className="mt-3">
                  <div className="flex justify-between text-[11px] text-muted-dark mb-1">
                    <span>
                      {regs.length} / {ev.capacity} registered
                    </span>
                    <span>{pct}%</span>
                  </div>
              ...
              <button
                ...
                className="flex items-center gap-1.5 text-xs font-semibold text-accent mt-3"
              >
