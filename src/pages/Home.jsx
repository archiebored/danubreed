...
//         <Link to="/signup" className="px-6 py-3 rounded-full bg-accent text-[#1a0a00] text-sm font-bold transition-all duration-200 hover:scale-[1.05] hover:shadow-lg hover:shadow-accent/30 active:scale-95">
//           Join us
//         </Link>
//         <Link to="/events" className="px-6 py-3 rounded-full border border-black/20 dark:border-white/20 text-sm font-bold transition-all duration-200 hover:scale-[1.05] hover:border-accent hover:text-accent active:scale-95">
//           See events
//         </Link>
//       </div>
//     </div>
...
//         <button onClick={prev} aria-label="Previous slide" className="absolute left-3 bottom-5 w-8 h-8 rounded-full bg-black/20 dark:bg-white/10 flex items-center justify-center backdrop-blur-sm text-white transition-transform duration-200 hover:scale-110 active:scale-90">
//           <ChevronLeft size={16} />
//         </button>
//         <div className="absolute bottom-5 left-1/2 -translate-x-1/2 flex gap-1.5">
//           {slides.map((_, i) => (
//             <button key={i} onClick={() => setCurrent(i)} aria-label={`Go to slide ${i + 1}`} className={`h-1.5 rounded-full transition-all duration-300 ${i === current ? 'w-6 bg-accent' : 'w-1.5 bg-white/30 hover:bg-white/50'}`} />
//           ))}
//         </div>
//         <button onClick={next} aria-label="Next slide" className="absolute right-3 bottom-5 w-8 h-8 rounded-full bg-black/20 dark:bg-white/10 flex items-center justify-center backdrop-blur-sm text-white transition-transform duration-200 hover:scale-110 active:scale-90">
...
//           </div>
//           <div className="aspect-[4/5] max-w-[260px] mx-auto w-full rounded-2xl bg-base-light dark:bg-base-dark border border-black/10 dark:border-white/10 flex items-center justify-center text-4xl overflow-hidden transition-transform duration-300 hover:-translate-y-1">
//             {featured?.photo_url ? (
//               <img src={featured.photo_url} alt={featured.full_name} className="w-full h-full object-cover" />
//             ) : (
//               '🙂'
//             )}
//           </div>
...
//           {serviceTimes.map((s) => (
//             <div key={s.id} className="relative overflow-hidden rounded-2xl border border-black/10 dark:border-white/10 bg-surface-light dark:bg-surface-dark p-6 transition-all duration-300 hover:-translate-y-1 hover:shadow-lg hover:shadow-accent/10 hover:border-accent/30">
...
//             <Link key={to} to={to} className="group relative overflow-hidden rounded-2xl border border-black/10 dark:border-white/10 bg-surface-light dark:bg-surface-dark p-5 text-center transition-all duration-300 hover:-translate-y-1.5 hover:shadow-lg hover:shadow-accent/10 hover:border-accent/30 active:scale-95">
//               <Icon size={22} className="text-accent mx-auto transition-transform duration-300 group-hover:scale-110 group-hover:-rotate-6" />
...
export default function Home() {
  ...
  return isStandalone ? <AppHome /> : <MarketingHome />;
}