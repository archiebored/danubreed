...
export default function StaffMembers() {
  ...
  function deptNames(memberId) {
    ...
    return ids
      .map((id) => departments.find((d) => d.id === id)?.name.replace(' Department', ''))
      .filter(Boolean)
      .join(', ');
  }
    ...
    <div>
      ...
      <div className="flex flex-col gap-2.5">
        {filtered.map((m) => (
          <div
            key={m.id}
            className="rounded-xl border border-white/10 bg-surface-dark p-4 flex flex-wrap items-center gap-4"
          >
            <div className="min-w-[160px]">
              <p className="text-sm font-semibold">{m.full_name}</p>
              <p className="text-xs text-muted-dark">
                Age {age(m.date_of_birth)} · joined {new Date(m.join_date).toLocaleDateString()}
              </p>
            </div>

            <div className="text-xs text-muted-dark min-w-[140px]">
              {deptNames(m.id) || 'No departments'}
            </div>

            <select
              value={m.tribe_id || ''}
              onChange={(e) => assignTribe(m.id, e.target.value)}
              className="rounded-md bg-base-dark border border-white/10 px-2 py-1.5 text-xs outline-none"
            >
            ...
            <button onClick={() => removeMember(m.id)} className="text-xs text-red-400 ml-auto font-medium">
              Remove
            </button>
          </div>
        ))}
        {!loading && filtered.length === 0 && (
          <p className="text-sm text-muted-dark">No members match that search.</p>
        )}
      </div>
    </div>
  );
